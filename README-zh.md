...
有些订阅源会拒绝 User-Agent 过于通用或不在白名单内的请求。你可以在 App Settings 中全局设置固定的 User-Agent，也可以在单个原始订阅源（Original Feed）的设置中单独覆盖。两处都留空则保持默认行为，即使用随机的浏览器 User-Agent；如果全局设置了固定值，而希望某个订阅源仍使用随机值，可在该订阅源中填入 `random`。
