...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if feed_name:
            try:
                ...
                raise CommandError(f"ProcessedFeed with name {feed_name} does not exist.") from None
        else:
    ...
    def format_digest(self, articles, what_to_include):
        ...
        if "include_toc" in what_to_include or (
            "include_one_line_summary" in what_to_include and any(article.summary_one_line for article in articles)
        ):
        ...
        if "include_content" in what_to_include or (
            "include_summary" in what_to_include and any(article.summary for article in articles)
        ):
            digest_builder.append("<br/>")
            ...
            for article in articles:
                ...
                digest_builder.append(f"<li><a href='{article.link}'>{article.title}</a></li>")
                if (
                    "include_toc" not in what_to_include
                    and "include_one_line_summary" in what_to_include
                    and article.summary_one_line
                ):
                    digest_builder.append(f"{article.summary_one_line}")
